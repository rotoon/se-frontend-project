...
// Global authentication middleware for admin pages
function checkAuthForAdminPages(req, res, next) {
  // Skip authentication check for login page, auth routes, and static files
  if (
    req.path === "/login" ||
  ...
  if (
    ...
    req.path.startsWith("/place-form")
  ) {
    if (!req.session || !req.session.user) {
      ...
      return res.redirect(
        "/login?error=" +
          encodeURIComponent("เซสชันหมดอายุ กรุณาเข้าสู่ระบบใหม่")
      );
    }
  }
...
// Protected places list route
app.get("/places", requireAuth, (req, res) => {
  res.sendFile(path.join(__dirname, "views/places-list.html"));
});

// Protected place form route
app.get("/place-form", requireAuth, (req, res) => {
  res.sendFile(path.join(__dirname, "views/place-form.html"));
});

// Protected place edit route
app.get("/place-form/:id", requireAuth, (req, res) => {
  res.sendFile(path.join(__dirname, "views/place-form.html"));
});
